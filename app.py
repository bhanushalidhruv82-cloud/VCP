...
# @app.route("/api/debug/yfinance/<symbol>")
# def debug_yfinance(symbol):
#     """Isolates whether yfinance/Yahoo connectivity works from this server,
#     independent of the DB. Hit this directly after deploying to see the
#     real error instead of a generic 500 from /api/watchlist/add."""
#     import traceback
...
# #     return df
# def _get_yf_session():
#     """Yahoo Finance blocks/rate-limits requests that look like they're
#     coming from a datacenter (e.g. Render, AWS) with a generic urllib
#     user-agent. Using a browser-like requests session with headers fixes
#     most 'works locally, breaks on Render' yfinance failures."""
#     import requests
...
#     except Exception as e:
#         print(f"[yfinance] {symbol} could not build custom session: {e}", flush=True)
...
#             df = df.reset_index()
#             print(f"[yfinance] {symbol} params={params} rows={len(df)} cols={list(df.columns)}", flush=True)
...
#         except Exception as e:
#             last_err = e
#             print(f"[yfinance] {symbol} failed with {params}: {type(e).__name__}: {e}", flush=True)

#     # Last-resort fallback: yf.Ticker uses a slightly different code path
#     # internally and occasionally succeeds when yf.download doesn't.
#     try:
#         print(f"[yfinance] {symbol} trying Ticker().history() fallback...", flush=True)
#         t = yf.Ticker(symbol, session=session) if session is not None else yf.Ticker(symbol)
#         df = t.history(period="max", auto_adjust=False, timeout=15)
#         if not df.empty:
#             df = df.reset_index()
#             if "Date" not in df.columns and "index" in df.columns:
#                 df = df.rename(columns={"index": "Date"})
#             print(f"[yfinance] {symbol} Ticker fallback rows={len(df)}", flush=True)
#             return df
#     except Exception as e:
#         last_err = e
#         print(f"[yfinance] {symbol} Ticker fallback failed: {type(e).__name__}: {e}", flush=True)

#     print(f"[yfinance] {symbol} returned no data. Last error: {last_err}", flush=True)
#     return pd.DataFrame()
...
#             conn.rollback()
#         import traceback
#         print(f"[watchlist/add] FAILED for {symbol}: {type(e).__name__}: {e}", flush=True)
...
@app.route("/api/ohlcv/batch", methods=["POST"])
def api_ohlcv_batch():
    """Returns OHLC (no volume - the screener doesn't need it) for many
    symbols in a single DB connection / single response, instead of the
    frontend firing one /api/ohlcv/<symbol> request per ticker. That old
    pattern was opening 300+ short-lived connections and serializing
    150-250KB of JSON per request back-to-back, which starved the single
    gunicorn worker long enough for Render's health check to time out and
    restart the dyno (visible as a wall of 502s mid-scan).

    Body: {"symbols": ["AAPL", "MSFT", ...], "watchlist": "Watchlist1"}
    Returns: {"AAPL": {"candles": [...]}, "MSFT": {"candles": [...]}, ...}
    Symbols with no data are simply omitted from the response.
    """
    data = request.json or {}
    symbols = data.get("symbols") or []
    symbols = [(s or "").strip().upper() for s in symbols if s]
    watchlist = data.get("watchlist", DEFAULT_WATCHLIST)
    ...
    try:
        with conn.cursor() as cur:
            # Single query for every requested symbol, streamed with
            # server-side cursor semantics via fetchmany to avoid holding
            # the whole 300-symbol result set in a Python list twice over.
            cur.execute(f"""
                SELECT s.symbol, o.date, o.open, o.high, o.low, o.close
                FROM {ohlcv_table} o
                JOIN {symbols_table} s ON s.id = o.symbol_id
                WHERE s.symbol = ANY(%s)
                ORDER BY s.symbol, o.date ASC;
            """, (symbols,))
            ...
            while True:
                ...
                for symbol, date, o, h, l, c in rows:
                    bucket = result.get(symbol)
                    if bucket is None:
                        bucket = result[symbol] = []
                    bucket.append({
                        "time": date.isoformat(),
                        "open": float(o),
                        "high": float(h),
        ...
        return jsonify({sym: {"candles": candles} for sym, candles in result.items()})
    finally:
        conn.close()
